...
| File | Contents |
|---|---|
| `source_ru.txt` | English source sentences |
| `reference_ru.txt` | Professional human translation into German |
| `system_1_ru.txt` | Output from a leading commercial NMT engine |
| `system_2_ru.txt` | Output from a local NMT engine |
| `system_3_ru.txt` | Output from an open-weight LLM (large) |
| `system_4_ru.txt` | Output from an open-weight LLM (medium / small) |
...
6. Click **Evaluate MT**.
7. Download the bar charts (PNG), the score tables (`mateo-evaluation.xlsx` and `mateo-evaluation-ci.xlsx`), and the sentence-level COMET file (`mateo-sentences.xlsx`) — we'll work through them together in Part 3.
